  ...
  return (
    <section id="gift" ref={ref} style={{background:'var(--cream-warm)',padding:'80px 40px',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{maxWidth:640,width:'100%',textAlign:'center'}}>
        ...
        <style>{`@media(max-width:640px){.gift-grid{grid-template-columns:1fr!important;}}`}</style>
        <div className="gift-grid reveal reveal-d2" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,textAlign:'left'}}>
          {accounts.map(acc => (
