...
// ── Gallery ───────────────────────────────────────────────────────
// Photos now ship with the static site under public/prewedding (served
// from the Cloudflare Pages CDN, $0 egress). The ordered file list lives
// in public/prewedding/manifest.json, generated by scripts/migrate-photos.mjs.
export async function fetchGalleryPhotos(): Promise<PhotoRow[]> {
  try {
    const res = await fetch('/prewedding/manifest.json', { cache: 'force-cache' })
    if (!res.ok) {
      console.error('[gallery] manifest fetch failed:', res.status)
      return []
    }
    ...
    return names
      ...
      .slice(0, 18)
      .map(name => ({ name, url: `/prewedding/${name}` }))
  } catch (e) {
